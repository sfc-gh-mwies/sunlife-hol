...
def main(session: Session) -> str:
   ...
   ####################################  
    ...
    if not table_exists(session, schema='ANALYTICS', name=username+'_DAILY_CITY_METRICS'):
        create_daily_city_metrics_table(session,username)
    
    merge_daily_city_metrics(session,username)
#    session.table('SNOWPARK_HOL_DB.ANALYTICS.DAILY_CITY_METRICS').limit(5).show()
...
def create_daily_city_metrics_table(session,username):
    ...
    dcm = session.create_dataframe([[None]*len(DAILY_CITY_METRICS_SCHEMA.names)], schema=DAILY_CITY_METRICS_SCHEMA) \
                        .na.drop() \
                        .write.mode('overwrite').save_as_table('SNOWPARK_HOL_DB.ANALYTICS.'+username+'_DAILY_CITY_METRICS')
    dcm = session.table('SNOWPARK_HOL_DB.ANALYTICS.'+username+'_DAILY_CITY_METRICS')


def merge_daily_city_metrics(session,username):
    ...
    print("{} records in stream".format(session.table('SNOWPARK_HOL_DB.HARMONIZED.'+username+'_ORDERS_STREAM').count()))
    orders_stream_dates = session.table('SNOWPARK_HOL_DB.HARMONIZED.'+username+'_ORDERS_STREAM').select(F.col("ORDER_TS_DATE").alias("DATE")).distinct()
    orders_stream_dates.limit(5).show()

    orders = session.table("SNOWPARK_HOL_DB.HARMONIZED."+username+"_ORDERS_STREAM").group_by(F.col('ORDER_TS_DATE'), F.col('PRIMARY_CITY'), F.col('COUNTRY')) \
...
#    orders.limit(5).show()
    ...
    weather_agg = weather.group_by(F.col('DATE_VALID_STD'), F.col('CITY_NAME'), F.col('COUNTRY_C')) \
                        .agg( \
                            ...
                            F.avg(F.call_udf("SNOWPARK_HOL_DB.ANALYTICS."+username+"_FAHRENHEIT_TO_CELSIUS_UDF", F.col("AVG_TEMPERATURE_AIR_2M_F"))).alias("AVG_TEMPERATURE_C"), \
                            F.avg("TOT_PRECIPITATION_IN").alias("AVG_PRECIPITATION_IN"), \
                            F.avg(F.call_udf("SNOWPARK_HOL_DB.ANALYTICS."+username+"_INCH_TO_MILLIMETER_UDF", F.col("TOT_PRECIPITATION_IN"))).alias("AVG_PRECIPITATION_MM"), \
